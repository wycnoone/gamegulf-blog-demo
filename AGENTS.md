...
- **`GameGulf` in the article body (markdown below frontmatter):** the detail page **main content** must include the substring `gamegulf` **at least three times** (case-insensitive) — e.g. the word **GameGulf** and/or `gamegulf.com` links in prose. **FAQ (`faq` in frontmatter) does not count** toward this rule; only the Markdown body after the closing `---` is validated. Title and description do not count. Use `scripts/one-off/inject-gamegulf-body-paragraph.mjs` to bulk-fix legacy posts if needed.
